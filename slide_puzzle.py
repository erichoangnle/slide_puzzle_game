...
def cut_image(img_name):
    ...
    """
    try:
        os.mkdir(pathlib.Path.cwd() / 'data' / 'temp')
    ...
    for i, j in grid:
        ...
        output = pathlib.Path.cwd() / 'data' / 'temp' / f'{int(i / tile_size)}-{int(j / tile_size)}.jpg'
        if output != (pathlib.Path.cwd() / 'data' / 'temp' / '5-3.jpg'):
...
def generate_puzzle(state):
    ...
    """
    tile_list = os.listdir(pathlib.Path.cwd() / 'data' / 'temp')
    tile_list.append("")
...
def render_game(img_name, state):
    ...
    for i in range(6):
        for j in range(4):
            try:
                tile = pygame.image.load(pathlib.Path.cwd() / 'data' / 'temp' / f'{state[i][j]}')
...
def main():
    ...
    while True:
        ...
        img_name = pathlib.Path.cwd() / 'data' / 'img' / f"{random.choice(os.listdir(pathlib.Path.cwd() / 'data' / 'img'))}"
        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.QUIT:
                    try:
                       rmtree(pathlib.Path.cwd() / 'data' / 'temp')
                    except FileNotFoundError:
                        pass
                    pygame.quit()
                    sys.exit()
